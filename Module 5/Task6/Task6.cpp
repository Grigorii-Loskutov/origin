...
// https://github.com/netology-code/sqlcpp-homeworks/tree/main/06
#include <iostream>
...
int main()
{
    ...
    try
    {
        ...
        try
        {
            ...
            publisher_added = session.add(std::move(p2));
            book_added = session.add(std::move(b2));
            shop_added = session.add(std::move(s2));
